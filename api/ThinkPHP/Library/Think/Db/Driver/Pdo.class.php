...
class Pdo extends Db{
    ...
    public function error() {
        ...
        if(defined('SHOW_DB_ERROR_REPORT') && SHOW_DB_ERROR_REPORT && $this->error){
        	
        	$art = 'SHOW COLUMNS FROM ';
        	if(!strstr($this->error, 'SHOW COLUMNS FROM ') && !strstr($this->error, 'DESCRIBE ')){
        		$this->__dump($this->error);
        		/* if(C('OPEN_DB_ERROR_MSG')){
        			$this->__dump($this->error);
        		}else{
        			__sent_mail($this->error);
        		} */
        	}
        }
